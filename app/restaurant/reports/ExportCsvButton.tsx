...
export default function ExportCsvButton({ report }: { report: RestaurantReport }) {
  function handleExport() {
    const rows: (string | number)[][] = [
      ...
      ['Descontos concedidos', '', report.totalDiscountGiven.toFixed(2)],
      ['Pedidos cancelados', report.cancelledOrders, ''],
      ['Pedidos recusados', report.rejectedOrders, ''],
      [],
      ['Forma de pagamento', 'Pedidos', 'Faturamento'],
      ...report.paymentMethodBreakdown.map((row) => [row.method, row.orders, row.revenue.toFixed(2)]),
      [],
